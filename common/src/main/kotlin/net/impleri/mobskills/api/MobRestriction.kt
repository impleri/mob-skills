...
class MobRestriction private constructor(
  registry: RestrictionsRegistry<Restriction>,
  fields: Array<Field>,
) : RestrictionsApi<EntityType<*>, Restriction>(registry, fields, MobSkills.LOGGER) {
  ...
  private val entityRestrictionsCache = HashMap<EntityType<*>, List<Restriction>>()
  private fun populateEntityRestrictions(entity: EntityType<*>): List<Restriction> {
    val isTargetingEntity = createPredicateFor(entity)
    return registry.entries()
      .filter { isTargetingEntity(it.target) }
  }

  private fun getRestrictionsFor(entity: EntityType<*>): List<Restriction> {
  ...
  private fun inIncludedSpawner(spawnType: MobSpawnType?): (Restriction) -> Boolean {
    return { it.includeSpawners.isEmpty() || it.includeSpawners.contains(spawnType) }
  }

  private fun notInExcludedSpawner(spawnType: MobSpawnType?): (Restriction) -> Boolean {
    return { it.excludeSpawners.isEmpty() || !it.excludeSpawners.contains(spawnType) }
  }

  private fun createExtraFilter(players: List<Player>): (Restriction) -> Boolean {
    return { !doesRestrictionAllowSpawn(it, players) }
  }

  companion object {
    internal val RestrictionRegistry = RestrictionsRegistry<Restriction>(MobSkills.MOD_ID)

    private val allRestrictionFields = Restriction::class.java.declaredFields

    internal val INSTANCE = MobRestriction(RestrictionRegistry, allRestrictionFields)
    ...
    fun add(name: ResourceLocation, restriction: Restriction) {
    ...
    private fun doesRestrictionAllowSpawn(restriction: Restriction, players: List<Player>): Boolean {
